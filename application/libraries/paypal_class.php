...
class Paypal_class {
	...
	public function submit_paypal_post() { 
		$paypal_url = ($this->ci->config->item('PayPalMode') == 'sandbox') ? $this->ci->config->item('sandboxpaypalurl') : $this->ci->config->item('livepaypalurl');
		echo "<html>\n";
 ...
 */
	public function validate_ipn() {
		...
		$paypal_url = ($_POST['test_ipn'] == 1) ? $this->ci->config->item('sandboxpaypalurl') : $this->ci->config->item('livepaypalurl');
