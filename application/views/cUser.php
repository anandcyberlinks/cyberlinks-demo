<aside class="content-wrapper">           
    <section class="content-header">
        <h1>
            ...
            <small><?php echo $welcome->loadPo('Customer') . " " . $welcome->loadPo('List') ?></small>&nbsp;&nbsp;<a href="<?php echo base_url() ?>user/register" class="btn btn-success btn-flat"><i class="fa fa-fw fa-plus-square"></i><?php echo $welcome->loadPo('Add New'); ?></a>
        </h1>
        <ol class="breadcrumb">
            ...
            <div class="col-md-12">
			    ...
			    <button class="btn btn-danger btn-sm" data-widget='collapse' data-toggle="tooltip" title="Collapse"><i class="fa fa-plus"></i></button>
                    ...
                    <div class="box-body" style="display:none;">
                        ...
                        <form  method="post" action="<?php echo base_url(); ?>customers/customer" id="searchIndexForm" name="searchIndexForm" accept-charset="utf-8">
                                <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-lg-4">
                                        <div class="input select">
                                            <label><?php echo $welcome->loadPo('Owner') ?></label>
                                            <select name="owner_id" class="form-control">
                                                <option value="">--Select--</option>
                                                <?php foreach($owner as $k=>$v){
                                                       ?>
                                                        <option value="<?=$v->id?>" <?= (isset($search['owner_id']) && $search['owner_id'] == $v->id) ? 'selected' : '' ?> ><?=$v->username?></option>
                                                       <?php
                                                       }
                                                ?>
                        ...
                        <table id="example2" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php foreach ($result as $value) { ?>
                                    <tr>
                                        <td><a href="<?= base_url() . 'customers/cprofile/' . $value->id ?>" class="profile"><?php echo $value->username; ?></a></td>
                                        <td><?php echo $value->first_name . " " . $value->last_name; ?></td>
                                        <td><?php echo $value->owner_name; ?></td>
                                        <td><?php echo $value->email; ?></td>
                                        <td><?php if (($value->status) == 'active') { ?>
                                                <a class="status" data="active" href="<?php echo base_url() ?>customers/changestatus/?id=<?php echo $value->id; ?>&email=<?= $value->email ?>&status=" title="Click To Inactive"><i class="fa fa-fw fa-check-circle-o"></i>Active&nbsp;&nbsp;</a>
                                            <?php } else {
                                                if (($value->status) == 'pending') {
                                                    echo "Pending";
                                                } else {
                                                    ?> 
                                                    <a class="status" data="inactive" href="<?php echo base_url() ?>customers/changestatus/?id=<?php echo $value->id; ?>&email=<?= $value->email ?>&domain=<?= $value->domain ?>&status=" title="Click To Active"><i class="fa fa-fw fa-circle-o"></i> Inactive</a>
                                                <?php } } ?>
                                        </td>
