...
class Ch_category extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Ch_category_model');
            ...
            if (isset($cid)) {
                if (isset($_POST['submit']) && $_POST['submit'] == "Update") {
                    ...
                    if ($this->form_validation->run()) {
                        ...
                        $this->Ch_category_model->_saveCategory($_POST);
                ...
                if (isset($_POST['submit']) && $_POST['submit'] == 'Submit') {
                    ...
                    if ($this->form_validation->run()) {
                        $result = $this->Ch_category_model->checkCategory($_POST['category'],$this->uid);
                        if ($result == 0) {
                            $this->Ch_category_model->_saveCategory($_POST);
    ...
    function uploadCategoryImg($tmpFilePath, $fileName, $id=null){
        ...
        if (!in_array($fileExt, $this->allowedImageExt)) {
            ...
            if($result){
                ...
                $fid = $this->Ch_category_model->_saveFile($fileData);
            }
        }
        return $fid;
    }
    ...
    public function delCatImage($file_id) {
        ...
        if($fileName) {
            ...
            $result = $this->Ch_category_model->delCategoryImage($file_id);
        }
    }
    ...
    function deleteCategory() {
        ...
        if ($per) {
            $id = $_GET['id'];
            $video = $this->Ch_category_model->getCategoryVideo($id);
            if ($video == '0') {
                $child = $this->Ch_category_model->getCategoryChild($id);
                if($child == '0'){
                    $this->Ch_category_model->delete_category($id);
